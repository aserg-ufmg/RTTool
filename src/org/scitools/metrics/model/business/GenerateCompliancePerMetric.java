...
//anterior
            ...
            //mudou a formula
            weigth[i] = Double.parseDouble(df.format((double) (Double.parseDouble(df.format(p1[i]).replace(",", ".")) + Double.parseDouble(df.format(p2[i]).replace(",", ".")))).replace(",", "."));
        }
        return weigth;
    }
                ...
                for (int i = 0; i < n2xMediana; i++) {
                    ...
                    for (String valorPercentil : colunaPercentil) {
                        if (flagAux) {
                            ...
                            flagAux = true;
                            if (i == 0) {
                                if (!(mediana == 0 && n2xMediana == 1)) {//caso haja só a primeira linha
                                    i++;//faz a primeira linha da contagem ser 0
                                }
                            }
                        }
